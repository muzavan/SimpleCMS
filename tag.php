...
<body>
        ...
        <div id="navbarCollapse" class="collapse navbar-collapse">
            ...
            <ul class="nav navbar-nav navbar-right">
            	<?php
					...
					?>
		                <li><a href="tag?tag=<?php echo str_replace(" ","", $row['category']);?>"><?php echo strtoupper($row['category']);?></a></li>
					<?php
							}
						}
				?>
		    ...
		    <div id="this-carousel-id" class="carousel slide"><!-- class of slide for animation -->
				  <div class="carousel-inner">
				 	<?php
			          	...
			          	$category = $_GET['tag'];
			          	$query = "SELECT * FROM `content` WHERE `category`=\"$category\" ORDER BY  `ID` DESC LIMIT 3;";
			          	$result = mysql_query($query);
	...
	<div class="container">
	    ...
	    <div id="recentNews"class="row">
	    	<?php
			          	...
			          	mysql_select_db('u160485837_bdg');
			          	$category = $_GET['tag'];
			          	$query = "SELECT * FROM `content` WHERE `category`=\"$category\" ORDER BY  `ID` DESC LIMIT 6;";
			          	$result = mysql_query($query);
			          	while($row = mysql_fetch_assoc($result)){
					?>
    ...
    <div class="container">
	    ...
	    <a href="#"><h2>POPULAR NEWS</h2></a>
	    <div id="recentNews"class="row">
	    	<?php
			          	mysql_connect('mysql.idhostinger.com','u160485837_bdg','connection');
			          	mysql_select_db('u160485837_bdg');
			          	$category = $_GET['tag'];
			          	$query = "SELECT * FROM `content` WHERE `category`=\"$category\" ORDER BY  `viewer` DESC LIMIT 6;";
			          	$result = mysql_query($query);
			          	while($row = mysql_fetch_assoc($result)){
					?>
					    <div class="col-md-4"><!-- class of active since it's the first item -->
